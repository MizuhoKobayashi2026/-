...
public class AdvancedStudentManagementSystem {
	...
	public AdvancedStudentManagementSystem() {
		...
		log.info("応用学生管理システムが初期化されました");
	}
	
	public void addStudent(Student student) throws StudentManagementException{
		if(student == null) {
			throw new StudentManagementException("学生はnullには出来ません");
		}
		
		if(students.containsKey(student.getId())) {
			throw new StudentManagementException("学生はすでに存在しています:" + student.getId());
		}
		
		students.put(student.getId(), student);
		gradeHistory.put(student.getId(), new ArrayList<>());
		
		log.info("学生が登録されました: {}({})", student.getName(), student.getStudentType());
	...
	public void addGrade(String studentId, Grade grade) 
		...
		log.info("{}の成績を追加しました: {}(Score: {})", student.getName(), grade.getSubject(), grade.getScore());
	}
